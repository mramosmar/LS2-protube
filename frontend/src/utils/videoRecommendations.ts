...
const SEMANTIC_GROUPS: { [key: string]: string[] } = {
    ...
    // Música
    musica: ['music', 'cancion', 'song', 'concierto', 'concert', 'banda', 'band', 'artista', 'artist', 'album', 'melodia', 'melody', 'ritmo', 'rhythm', 'instrumento', 'instrument', 'guitarra', 'guitar', 'piano'],

    // Tecnología
    tecnologia: ['technology', 'tech', 'software', 'hardware', 'programacion', 'programming', 'codigo', 'code', 'ordenador', 'computer', 'movil', 'mobile', 'app', 'aplicacion', 'internet', 'web', 'digital'],

    // Viajes
    viaje: ['travel', 'trip', 'vacation', 'vacaciones', 'turismo', 'tourism', 'ciudad', 'city', 'pais', 'country', 'destino', 'destination', 'aventura', 'adventure', 'mundo', 'world', 'explorar', 'explore'],

    // Gaming
    gaming: ['game', 'juego', 'videojuego', 'videogame', 'gamer', 'jugador', 'player', 'gameplay', 'stream', 'streaming', 'esports', 'competitivo', 'competitive'],

    // Educación
    educacion: ['education', 'tutorial', 'clase', 'class', 'curso', 'course', 'aprender', 'learn', 'ensenar', 'teach', 'leccion', 'lesson', 'estudiar', 'study', 'explicacion', 'explanation'],

    // Arte y creatividad
    arte: ['art', 'arte', 'dibujo', 'draw', 'drawing', 'dibuja', 'pintura', 'paint', 'painting', 'pintar', 'diseno', 'design', 'creativo', 'creative', 'artista', 'artist', 'obra', 'work', 'ilustracion', 'illustration', 'sketch', 'color', 'colours', 'colors', 'colores', 'acuarela', 'watercolor', 'watercolour', 'lapiz', 'pencil', 'brush', 'pincel', 'lienzo', 'canvas', 'study', 'studies', 'estudio', 'estudios', 'principiantes', 'beginners', 'beginner'],

    // Entretenimiento
    entretenimiento: ['entertainment', 'diversion', 'fun', 'diversión', 'comedia', 'comedy', 'humor', 'gracioso', 'funny', 'risa', 'laugh', 'show', 'espectaculo', 'spectacle'],

    // Naturaleza
    naturaleza: ['nature', 'natural', 'animal', 'animales', 'animals', 'planta', 'plant', 'bosque', 'forest', 'montana', 'mountain', 'mar', 'sea', 'oceano', 'ocean', 'paisaje', 'landscape'],

    // Moda y belleza
    moda: ['fashion', 'style', 'estilo', 'ropa', 'clothes', 'outfit', 'tendencia', 'trend', 'belleza', 'beauty', 'maquillaje', 'makeup', 'pelo', 'hair', 'look'],

    // Ciencia
    ciencia: ['science', 'scientific', 'cientifico', 'experimento', 'experiment', 'investigacion', 'research', 'descubrimiento', 'discovery', 'teoria', 'theory', 'fisica', 'physics', 'quimica', 'chemistry', 'biologia', 'biology'],
};
...
export function calculateTitleSimilarity(video1: Video, video2: Video): number {
    ...
    // Normalizar y dividir en palabras
    const words1 = video1.title.toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .split(/\s+/)
        .filter(word => word.length > 2 && !stopWords.has(word));

    const words2 = video2.title.toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .split(/\s+/)
        .filter(word => word.length > 2 && !stopWords.has(word));
